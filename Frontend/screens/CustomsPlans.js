...
import { View, Image, Text, TouchableOpacity, StyleSheet, Modal, Alert, TextInput, Switch, ScrollView, Keyboard } from 'react-native';
...
const CustomsPlan = ({ navigation }) => {
    ...
    const products = [
        ...
        { id: 3, name: 'Cheese', price: 'Rs 600/KG', image: require('../assets/HomePageImages/cheese.jpg') },
        { id: 4, name: 'Paneer', price: 'Rs 500/KG', image: require('../assets/HomePageImages/paneer.jpg') },
    ];
    ...
    const overallTotalPrice = totalMilkPrice + totalBreadPrice + totalCheesePrice + totalPaneerPrice;
    ...
    const handleAddToCart = () => {
        Alert.alert("Products added to cart!", `Milk: ${milkCount}, Bread: ${breadCount}, Cheese: ${cheeseCount}, Paneer: ${paneerCount}`);
    };
                            ...
                            <View style={styles.infoContainer}>
                                ...
                                <View style={styles.counterContainer}>
                                    ...
                                    <Text style={styles.productCount}>
                                        {product.id === 1 ? milkCount : product.id === 2 ? breadCount : product.id === 3 ? cheeseCount : paneerCount}
                                    </Text>
    ...
    <View style={styles.countRow}>
        <View style={styles.productCountContainer}>
            ...
            <Text style={styles.countText}>Milk Count: {milkCount}</Text>
        </View>
        <View style={styles.productCountContainer}>
            <Image style={styles.smallImage} source={products[1].image} />
            <Text style={styles.countText}>Bread Count: {breadCount}</Text>
        </View>
    </View>
    <View style={styles.countRow}>
        <View style={styles.productCountContainer}>
            <Image style={styles.smallImage} source={products[2].image} />
            <Text style={styles.countText}>Cheese Count: {cheeseCount}</Text>
        </View>
        <View style={styles.productCountContainer}>
            <Image style={styles.smallImage} source={products[3].image} />
            <Text style={styles.countText}>Paneer Count: {paneerCount}</Text>
        </View>
    </View>
</View>
                ...
                {/* <View style={styles.countContainer}>
                    {products.map((product) => (
                        <View key={product.id} style={styles.countRow}>
                            <Image style={styles.smallImage} source={product.image} />
                            <Text style={styles.countText}>{product.name} 
                            Count: {product.id === 1 ? milkCount : product.id === 2 ? 
                            breadCount : product.id === 3 ? cheeseCount : paneerCount}</Text>
...
/>
                ...
                />
                <TouchableOpacity style={[styles.input, { zIndex: 1 }]} onPress={showDatePicker}>
                    <Text>{skipDays.length > 0 ? skipDays.join(', ') : "Select Skip Days"}</Text>
                </TouchableOpacity>
                <DateTimePickerModal
            ...
            <ScrollView style={styles.priceSummary}>
                ...
                <Text style={styles.priceSummaryItem}>Milk Price: Rs {totalMilkPrice}</Text>
                <Text style={styles.priceSummaryItem}>Bread Price: Rs {totalBreadPrice}</Text>
                <Text style={styles.priceSummaryItem}>Cheese Price: Rs {totalCheesePrice}</Text>
                <Text style={styles.priceSummaryItem}>Paneer Price: Rs {totalPaneerPrice}</Text>
                <Text style={styles.totalPrice}>Overall Total Price: Rs {overallTotalPrice}</Text>
